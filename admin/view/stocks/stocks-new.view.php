    ...
    <section class="content-header">
     ...
     <div class="container-fluid d-flex row-md-12">
      <!-- <div class="card col-md-12">
        <h1><center>jsahsjahsjahsja</center></h1>
      </div> -->
     </div>
      <div class="container-fluid d-flex row-md-12">
      ...
      <div class="container col-md-6" >
        <div class="card shadow p-3" >
              ...
              <div class="card-header mb-0">
                      <div class="row mb-2">
                        <div class="col-md-3">
                          <a href="index.php?pg=stocks" class="btn btn-warning px-5">Back</a>
                        </div>
                        <div class="col-md-5">
                          <h4 class="fw-bold text-right text-secondary" style="font-weight: bolder;"><strong><i class="fa fa-search"></i> Search Product</strong></h4>
                        </div>
                        <div class="col-md-1"></div>
                        <div class="col-md-3">
                        <button onclick="clear_all()" class="col-12 btn btn-danger px-3">Clear-all</button>
                        </div>
                      </div>
                    </div>
                    <div class="row">
              ...
              <table id="myTable1" class="table table-sm table-bordered table-striped table-hover bg-light mb-0 text-center">
        ...
        <div class="card-header mb-0">
                      <div class="row mb-2">
                      <div class="col-md-6">
                          <h4 class="fw-bold text-secondary text-left" style="font-weight: bolder;"><strong><i class="fa fa-arrow-up"></i> Add Quantity </strong></h4>
                          
                        </div>
                        <div class="col-sm-4"></div>
                        <div class="col-md-2">
                          <button type="button" class="btn btn-primary px-4" onclick="stocked(event)">Complete</button>
                        </div>
                      </div>
                      </form>
        </div>
                <div class="row-md-5">
                        <center><h4 class="fw-bold text-secondary" style="font-weight: regular; font-size: 20px;">Product Count <div class="js-items-count badge badge-sm bg-warning rounded-circle mb-1">0</div></h4></center>
                      </div>
  ...
  <script>
   ...
   function search_item(e){
    ...
    function product_html(data, index){
      return `    
                  <tr class="text-center">
                          ...
                          <td index="${index}" style="cursor: pointer">${data.barcode}</td>
                          <td index="${index}" style="cursor: pointer" class="w-50">${data.description}</td>
                          <td index="${index}" style="cursor: pointer">${data.cost}</td>
                          <td index="${index}" style="cursor: pointer">${data.price}</td>
                          <td index="${index}" style="cursor: pointer">${data.qty}</td>
                    </tr>    
                                `;
    }
    function item_html(data,index){
        return `
        <tr class="text-center">
                          <td index="${index}">${data.barcode}</td>
                          <td index="${index}" class="w-50">${data.description}</td>
                          <td class="d-flex border-0 col-6" style="width:100px">
                          <input index="${index}" onblur="change_qty('input', event)" class="text-left form-control" value="${data.qty}" style="width:108px" type="number" onclick="this.select()">
                          <div class="px-1"><button onclick="clear_item(${index})" class="btn btn-sm btn-danger mt-1 px-1" style="width:30px"><i class="fa fa-times"></i></button></div>
                          </td> 
                                `;
    }
